...
In CI, `.github/workflows/e2e.yml` triggers on every PR (plus the nightly schedule and manual `workflow_dispatch`) and always reports both check runs. An `e2e-plan` job computes `run_app`/`run_gov` from changed files (`apps/app.mento.org/**` + `packages/web3/**` -> `run_app`; `apps/governance.mento.org/**` + `packages/web3/**` -> `run_gov`; root-level files like `package.json`/`turbo.json`/the workflow itself -> both) and fast-no-ops the fork jobs to a green skip when their surface didn't change — that "always reports" property is the prerequisite for eventually adding these checks to the required-checks ruleset (`strict_required_status_checks_policy` would otherwise deadlock non-matching PRs). A cheap `fork-seed-self-test` job (no anvil, no network) runs unconditionally and gates both fork jobs so a broken encoder fails in ~15s instead of after a 30-minute anvil run. `e2e-connected` ("Connected swap (anvil fork)") and `e2e-governance` ("Connected governance (anvil fork)") both fork Celo mainnet pinned to `FORK_BLOCK` (bump roughly monthly). The fork source is a keyless public archive RPC probed at run time — forno cannot serve pinned-block forks because it prunes a block's state within minutes. A nightly scheduled run (04:20 UTC) forces both plan outputs true and repeats the suites at a freshly resolved recent block instead of the pin, to catch chain drift (oracle config, pool, or contract changes) that plan-gated PR runs never see. Neither check is a required check yet.
