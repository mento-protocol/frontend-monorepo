...
export function DebtInput({ value, onChange, collAmount }: DebtInputProps) {
  ...
  return (
    <div className="gap-2 flex flex-col">
      ...
      />
      {minDebtFormatted && (
        <p className="text-xs text-muted-foreground">Min: {minDebtFormatted}</p>
      )}
        ...
        <div className="gap-2 flex flex-wrap">
          {suggestions.map((s) => (
            <button
              key={s.risk}
              type="button"
              className="px-3 py-1 text-xs font-medium cursor-pointer rounded-full border border-input bg-background transition-colors hover:bg-accent hover:text-accent-foreground"
