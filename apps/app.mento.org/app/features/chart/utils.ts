import {
  MAX_TOKEN_PRICE_NUM_DAYS,
  STALE_TOKEN_PRICE_TIME,
...
import { areDatesSameDay } from "@/lib/utils/time";

export function tokenPriceHistoryToChartData(
  prices: TokenPriceHistory | undefined,
  numDays = 7,
): ChartData {
  let dataValues = generateChartData(numDays, (date: Date) =>
    findPriceForDay(prices, date),
  );
...
export function findPriceForDay(
  prices: TokenPriceHistory | undefined,
  date: Date,
) {
...
export function findMissingPriceDays(
  numDays: number,
  prices?: QuoteCurrencyPriceHistory,
) {
  const now = new Date();
  ...
  if (prices && prices[TokenId.cUSD]) {
    for (const p of prices[TokenId.cUSD]!) {
      ...
      if (
        pDate !== now.getDate() ||
        now.getTime() - p.timestamp < STALE_TOKEN_PRICE_TIME
      ) {
...
export function mergePriceHistories(
  ...
  for (const key of Object.keys(TokenId)) {
    ...
    const oldPricesFiltered = oldPrices.filter((p) => {
      ...
      if (newPrices.find((np) => new Date(np.timestamp).getDate() === day))
        return false;
      return true;
    });
