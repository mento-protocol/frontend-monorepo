...
describe("swap utilities", () => {
  describe("parseInputExchangeAmount", () => {
    it("Parses wei amount", () => {
      expect(
        parseInputExchangeAmount(ONE_ETH_WEI, TokenId.CELO, true).toString(),
      ).toEqual(ONE_ETH_WEI);
    });
    it("Parses ether amount", () => {
      expect(
        parseInputExchangeAmount("12.345", TokenId.CELO, false).toString(),
      ).toEqual("12345000000000000000");
    });
    it("Falls back to 0", () => {
      expect(
        parseInputExchangeAmount("invalid", TokenId.CELO, true).toString(),
      ).toEqual("0");
    });
  });
  ...
  describe("getMinBuyAmount", () => {
    it("Computes min buy for small slippage", () => {
      expect(getMinBuyAmount(ONE_ETH_WEI, 0.5).toString()).toEqual(
        "995000000000000000",
      );
    });
  ...
  describe("getMaxSellAmount", () => {
    it("Computes max sell for small slippage", () => {
      expect(getMaxSellAmount(ONE_ETH_WEI, 0.5).toString()).toEqual(
        "1005000000000000000",
      );
    });
  ...
  describe("calcExchangeRate", () => {
    it("Calculates rate for tokens with equal decimals", () => {
      expect(calcExchangeRate(ONE_ETH_WEI, 18, HALF_ETH_WEI, 18)).toEqual(
        "2.0000",
      );
    });
    it("Calculates rate for tokens with different decimals", () => {
      expect(calcExchangeRate(ONE_ETH_WEI, 18, ONE_ETH_WEI, 17)).toEqual(
        "0.1000",
      );
