import { MIN_ROUNDED_VALUE } from "@/config/consts";
import { useCallback } from "react";
// Tokens and getTokenByAddress are used as values, AppTokenId as type
import { getMentoSdk, getTradablePairForTokens } from "@/features/sdk";
import type { TokenId as AppTokenId } from "@/config/tokens";
...
import { parseAmount, toWei } from "@/utils/amount";
...
async function checkTradingLimits(
  ...
  const tradingLimits = await mento.getTradingLimits(exchangeId as string);
  ...
  const tokenToCheck = getTokenByAddress(
    tradingLimits[0].asset as Address,
  ).symbol;
