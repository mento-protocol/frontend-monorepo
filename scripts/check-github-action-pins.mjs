...
const ALLOWED_POLICY_PNPM_VERSIONS = ["10.34.4", "10.34.5"];
const CANONICAL_POLICY_PNPM_VERSION = ALLOWED_POLICY_PNPM_VERSIONS[0];
const observedPolicyPnpmVersions = new Map();
 ...
 * can move to the fixed release. Normalize only the exact Setup PNPM field
 * after validating its temporary allowlist. Every other workflow value remains
 * part of the strict structural comparison.
 * @param {string} path
...
function normalizePolicyPnpmVersion(path, workflow) {
  ...
  observedPolicyPnpmVersions.set(path, version);
...
if (
  REQUIRED_POLICY_FILES.every((path) => observedPolicyPnpmVersions.has(path)) &&
  new Set(observedPolicyPnpmVersions.values()).size !== 1
) {
  failures.push({
    file: REQUIRED_POLICY_FILES.join(","),
    line: 1,
    problem:
      "invalid action-pin policy workflow (both workflows must use the same Setup PNPM version)",
  });
}
