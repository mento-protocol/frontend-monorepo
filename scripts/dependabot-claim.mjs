...
const {
  DEPENDABOT_CLAIM_POLICY_REF: policyRefOverride,
  DEPENDABOT_CLAIM_PACKAGE_DIR: localPackageDirectory,
} = { ...process.env };
...
function gitText(argv, cwd, description) {
  ...
  if (result.status !== 0) {
    fail(
      EXIT_CONFIG,
      `${description}: ${result.stderr.toString("utf8").trim() || `git exited ${result.status}`}`,
    );
  }
...
if (!POLICY_REF.test(policyRef)) {
  fail(
    EXIT_CONFIG,
    `DEPENDABOT_CLAIM_POLICY_REF must be a git ref or object id, found ${policyRef}`,
  );
}
...
const policyBlob = git(
  ["cat-file", "blob", `${policyOid}:${POLICY_PATH}`],
  repositoryRoot,
);
if (policyBlob.status !== 0) {
  fail(
    ...
    `cannot read ${POLICY_PATH} at ${policyRef} (${policyOid}): ${
      policyBlob.stderr.toString("utf8").trim() ||
      `git exited ${policyBlob.status}`
    }`,
  );
}
...
try {
  ...
  fail(
    EXIT_CONFIG,
    `cannot parse ${POLICY_PATH} at ${policyRef} (${policyOid}): ${error.message}`,
  );
}
...
function localBinary(directory) {
  ...
  try {
    ...
    fail(
      EXIT_CONFIG,
      `cannot read ${join(packageRoot, "package.json")}: ${error.message}`,
    );
  }
...
if (localPackageDirectory) {
  process.stderr.write(
    `dependabot:claim: running ${pin.name}@${pin.version} from ${resolve(localPackageDirectory)}\n`,
  );
}
